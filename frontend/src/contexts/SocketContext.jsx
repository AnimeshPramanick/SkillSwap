...
export const SocketProvider = ({ children }) => {
  ...
  const setupSocketListeners = (socket) => {
    ...
    socket.on("incoming_video_call", (callData) => {
      ...
      toast(`${callData.callerName} is calling you...`, { icon: "📞" });
    });
  ...
  const handleNewMessage = (message) => {
    ...
    window.dispatchEvent(
      new CustomEvent("socket_message", { detail: message })
    );
  ...
  const handleMessageSent = (data) => {
    window.dispatchEvent(
      new CustomEvent("socket_message_sent", { detail: data })
    );
  };
  ...
  const handleMessageHistory = (data) => {
    window.dispatchEvent(
      new CustomEvent("socket_message_history", { detail: data })
    );
  };
  ...
  const handleNotification = (notification) => {
    ...
    // Emit custom event
    window.dispatchEvent(
      new CustomEvent("socket_notification", { detail: notification })
    );
  };
  ...
  const value = {
    ...
    typingUsers: Array.from(typingUsers.entries()).map(
      ([userId, timestamp]) => ({
        userId,
        timestamp,
      })
