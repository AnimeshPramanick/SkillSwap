...
const { getUser, updateUser, createMessage, getConversationMessages } = require('../config/firebase');
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('get_message_history', async (data) => {
      try {
        ...
        const messages = await getConversationMessages(socket.userId, recipientId, limit);
