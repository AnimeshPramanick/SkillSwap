    ...
    try {
      ...
      const skillCompatible =
        ((organizerSkills.desired && organizerSkills.desired.includes(skill)) ||
          (organizerSkills.teachable &&
            organizerSkills.teachable.includes(skill))) &&
        ((participantSkills.desired &&
          participantSkills.desired.includes(skill)) ||
          (participantSkills.teachable &&
            participantSkills.teachable.includes(skill)));
      ...
      const sessionData = {
        ...
        sessionRoom: `session_${[organizerId, participantId]
          .sort()
          .join("_")}_${Date.now()}`,
      };
...
router.patch(
  ...
  [
    body("status")
      ...
      .withMessage(
        "Status must be scheduled, confirmed, in-progress, completed, or cancelled"
      ),
    body("scheduledAt")
      .optional()
      .isISO8601()
  ...
  async (req, res) => {
    try {
      ...
      if (req.body.scheduledAt !== undefined)
        updates.scheduledAt = req.body.scheduledAt;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (participantId === organizerId) {
        return res.status(400).json({
          error: "Cannot create instant meeting with yourself",
        });
      }
      ...
      if (io) {
        io.to(`user_${participantId}`).emit("instant_meeting_invite", {
          sessionId,
          organizerId,
          organizerName: req.user.profile?.name || req.user.username,
