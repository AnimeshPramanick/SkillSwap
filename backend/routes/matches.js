  ...
  try {
    ...
    if (userDesiredSkills.length === 0 && userTeachableSkills.length === 0) {
      return res.status(400).json({
        error:
          "Please add some skills to your profile before searching for matches",
      });
    }
    ...
    const scoredMatches = potentialMatches.map((user) => {
      ...
      const compatibilityScore =
        desiredMatchScore * 0.6 + teachableMatchScore * 0.4;
      ...
      return {
        ...
        skillAlignment: {
          theyCanTeachMe: userDesiredSkills.filter((skill) =>
            userTeachable.includes(skill)
          ),
          theyWantToLearn: userTeachableSkills.filter((skill) =>
            userDesired.includes(skill)
          ),
        },
      };
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let existingMatches = [];
      try {
        existingMatches = await getUserMatches(currentUserId);
      } catch (error) {
        // If index doesn't exist yet, we'll skip the duplicate check
        console.warn(
          "Could not check for existing matches (index may not exist):",
          error.message
        );
      }
      ...
      const matchData = {
        ...
        compatibility: {
          ...
          user2DesiredMatches: targetDesired.filter((skill) =>
            currentTeachable.includes(skill)
          ),
          totalSharedInterests:
            currentDesired.filter((skill) => targetTeachable.includes(skill))
              .length +
...
router.get("/suggestions/refresh", verifyToken, async (req, res) => {
  try {
    ...
    const potentialMatches = await searchUsers({
      isOnline: undefined,
      limit: 100,
    });
    const currentUser = await getUser(req.user.id);
